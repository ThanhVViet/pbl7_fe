...
const AddProduct = () => {
    ...
    const formik = useFormik({
        ...
        onSubmit: (values) => {
            let parsedSpecs = {};
            try {
                parsedSpecs = values.specs ? JSON.parse(values.specs) : {};
            } catch (e) {
                toast.error("Thông số kỹ thuật không đúng định dạng JSON!");
                return;
            }
            const payload = {
                ...values,
                ...
                <Grid2 container spacing={2}>
                    <Grid2 size={{xs: 12}} className='flex flex-row items-start gap-3'>
                        ...
                        <label htmlFor='fileInput'
                               className='relative w-32 h-32 flex items-center justify-center border-2 border-dashed border-gray-300 rounded-lg cursor-pointer bg-gray-100 hover:bg-gray-200 transition'>
                            ...
                            {formik.values.images.map((image, index) => (
                                <div className='relative group' key={index}>
                                    ...
                                    <IconButton onClick={() => handleRemoveImage(index)} size='small' color='error'
                                                className='!absolute top-0 right-0 bg-white/80 group-hover:bg-white'>
                                        <Close sx={{fontSize: "1rem"}}/>
                                    </IconButton>
                                </div>
                            ))}
                        </div>
                        {/* <span className='text-xs text-gray-400 mt-1'>Chọn nhiều ảnh để hiển thị sản phẩm nổi bật hơn</span> */}
                    </Grid2>

                    <Grid2 size={{xs: 12, lg: 6}}>
                        <TextField fullWidth id='title' name='title' label='Tên sản phẩm'
                    ...
                    <Grid2 size={{xs: 12, md: 4, lg: 3}}>
                        <TextField fullWidth id='sellingPrice' name='sellingPrice' label='Giá của sản phẩm'
                                   value={formik.values.sellingPrice} type='number'
                                   onChange={formik.handleChange}
                    ...
                    <Grid2 size={{xs: 12, md: 4, lg: 3}}>
                        <TextField fullWidth id='quantity' name='quantity' label='Số lượng'
                    ...
                    <Grid2 size={{xs: 12, md: 4, lg: 3}}>
                        ...
                        >
                            ...
                            <Select labelId='category-label' id='category' name='categoryId'
